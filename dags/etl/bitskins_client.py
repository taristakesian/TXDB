...
class BitSkinsClient:
    def __init__(self, amount_skins_unload: int = 10, api_key: str = BITSKINS_API_KEY):
        ...
        import os
        print(os.getcwd())
        self.id_unload = yaml.safe_load(open("/opt/airflow/dags/etl/config/id_unload.yaml"))['id']

    def get_items(self):
        bd = []
        time_for_req = 1
        for i in (self.id_unload[:self.amount_skins_unload]):
            data = {
                "limit": 100,
                "offset": 0,
                "where": {"skin_id": [i]},
